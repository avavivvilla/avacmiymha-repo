...
def read_annotation(tif_path):
    ...
    with tifffile.TiffFile(tif_path) as tf:
        ...
        #changed index since the raw image was on top layer not bottom - ava
...
manual_layer = manual_layer [:,:,:3] #gets rid of extra channel in manual_layer - ava
img = img.astype(np.uint8)
manual_mask = manual_layer > 0        # foreground = any non-zero pixel
print("Read annotation:", tif_path)
imgshape = img.shape #need these dimensions for later - ava
print("  raw layer:", imgshape, " manual layer:", manual_layer.shape)
...
ax = ax.ravel()
#img = img.astype(np.short)
ax[0].imshow(img.astype(np.short), cmap="gray");         ax[0].set_title("Raw (from TIFF)")
ax[1].imshow(flipped.astype(np.short), cmap="gray");     ax[1].set_title("1. Flipped intensities")
ax[2].imshow(255*foreground.astype(np.short), cmap="gray");  ax[2].set_title("2. Foreground (bg threshold)")
ax[3].imshow(255*tissue.astype(np.short), cmap="gray");      ax[3].set_title("3. Two largest components")
ax[4].imshow(255*cortex.astype(np.short), cmap="gray");      ax[4].set_title(f"4. Cortex - Otsu\nDice={dice:.3f}")
overlay = np.zeros((imgshape[0],imgshape[1], 3))

#defined the function to convert the 3-colored channel image to 1-colored channel grayscale - ava
def rgb2gray(imin):
    return imin[:,:,0]+imin[:,:,1]+imin[:,:,2]
#make both cortex and manual mask black and white - ava
cortex_gray =255*(rgb2gray(cortex) > 0).astype(np.short)

manual_mask_gray = 255*(rgb2gray(manual_mask).astype(np.short))

overlay[..., 0] = cortex_gray               # red   = automatic
overlay[..., 1] = manual_mask_gray          # green = manual
